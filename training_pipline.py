...
from eval_functions import calculate_SER, calculate_CER, calculate_BER
    ...
    def evaluate(model, datamodule):
        ...
        with torch.no_grad():
            for batch in datamodule.test_dataloader():
                ...
                ser = calculate_SER(outputs, batch)
                cer = calculate_CER(outputs, batch)
                ber = calculate_BER(outputs, batch)
                print(f"SER: {ser}, CER: {cer}, BER: {ber}")

    evaluate(model, datamodule)
